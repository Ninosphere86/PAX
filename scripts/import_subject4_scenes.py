...
QUESTION_RE = re.compile(r"^C03_(\d{2,3})\s+(.*)$")
...
def validate(questions, answer_table):
    ...
    if actual != [1, 2, 3, 4, 5]:
        errors.append(f"题号不连续：expected 1..5, actual {actual}")
    ...
    if len(answer_table) != 5:
        errors.append(f"答案速查表数量应为 5，实际为 {len(answer_table)}")
    if errors:
        raise ValueError("\n".join(errors))
...
def build_records(questions, image_dir):
    ...
    for question in questions:
        number = question["number"]
        code = f"C03_{number:02d}"
        image_name = f"c03-{number:02d}-v2.webp"
        if not (image_dir / image_name).is_file():
            raise FileNotFoundError(f"缺少重制题图：{image_name}")

        explanation = question["explanation"]
        if code == "C03_05" and explanation == "辆在非紧急情况下不得占用":
            explanation = "车辆在非紧急情况下不得占用应急车道"

        records.append(
            {
                "id": f"subject4-scenes-{code}",
                "code": code,
...
def main():
    parser = argparse.ArgumentParser(description="导入科目四场景题 Word 题库")
    ...
    if args.apply:
        ...
        bank = [
            item
            for item in bank
            if item.get("section") != "C03" and not item.get("code", "").startswith("C03_")
        ]
        bank.extend(records)
